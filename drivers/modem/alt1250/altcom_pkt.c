...
{
  int valid_version = ALTCOM_CMD_VER_UNKNOWN;
  ...
  if (hdr->ver == ALTCOM_CMD_VER_V1)
    {
      ...
      if ((hdr->v1_checksum == csum.header_checksum)
        ...
        {
          valid_version = ALTCOM_CMD_VER_V1;
        }
      else
        {
  ...
  else if(hdr->ver == ALTCOM_CMD_VER_V4)
    {
      if ((hdr->v4_hdr_cksum == csum.header_checksum)
        ...
        {
          valid_version = ALTCOM_CMD_VER_V4;
        }
      else
        {
  ...
  struct calculated_checksum csum)
{
  ...
  if (valid_version == ALTCOM_CMD_VER_V1)
    {
      if ((hdr->cmdid == (ALTCOM_CMDID_POWER_ON_V1 | ALTCOM_CMDID_REPLY_BIT))
        ...
        {
          valid_version = ALTCOM_CMD_VER_UNKNOWN;
        }
    }
  else if(valid_version == ALTCOM_CMD_VER_V4)
    {
...
bool altcom_is_v1pkt_ok(struct altcom_cmdhdr_s *cmdhdr)
{
  ...
  return (check_valid_reply(cmdhdr, checksum) == ALTCOM_CMD_VER_V1)
...
bool altcom_is_v4pkt_ok(struct altcom_cmdhdr_s *cmdhdr)
{
  ...
  checksum = convert_ntoh(cmdhdr);
  return (check_valid_reply(cmdhdr, checksum) == ALTCOM_CMD_VER_V4)
    && (cmdhdr->payload[0] == LTE_RESULT_OK);
}
...
FAR void *altcom_make_poweron_cmd_v1(int *sz)
{
  ...
  set_header_top(hdr, ALTCOM_CMD_VER_V1, ALTCOM_CMDID_POWER_ON_V1);
...
FAR void *altcom_make_poweron_cmd_v4(int *sz)
{
  ...
  set_header_top(hdr, ALTCOM_CMD_VER_V4, ALTCOM_CMDID_POWER_ON_V4);
